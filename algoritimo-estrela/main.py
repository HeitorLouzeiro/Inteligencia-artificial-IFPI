...
def aestrela(labirinto):
    ...
    f_score[celula_inicial] = g_score[celula_inicial] + \
        h_score(celula_inicial, destino)
    print(f_score)

    fila = PriorityQueue()
    item = (f_score[celula_inicial], h_score(
        celula_inicial, destino), celula_inicial)
    fila.put(item)
        ...
        for direcao in "NSEW":
            if labirinto.maze_map[celula][direcao] == 1:
                ...
                if novo_f_score < f_score[proxima_celula]:
                    ...
                    item = (novo_f_score, h_score(
                        proxima_celula, destino), proxima_celula)
                    fila.put(item)
